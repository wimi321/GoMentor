...
test('v5 teaching runtime wires motifs, evidence, verification, and multilingual settings', async () => {
  ...
  assert.match(teacher, /runTeacherAgentSession/)
  assert.match(teacher, /knowledgeBundleForState/)
  assert.match(teacher, /searchKnowledgeMatches/)
  assert.match(teacher, /recommendedProblemsFromMatches/)
  assert.doesNotMatch(evidence, /buildHumanTeacherInstruction/)
  assert.doesNotMatch(evidence, /friendlyTeacherFallback/)
  assert.match(teacher, /agentSystemPrompt/)
